...
const Page = () => {
    ...
    const handleApply = (jobId, job) => {
        setSelectedJobId(jobId);        // Keep the selectedJobId for backend submission
        setSelectedJob(job);            // Set the selected job details, including jobTitle
        setIsApplyModalOpen(true);      // Open the Apply modal
    };
    ...
    const handleSubmit = async (formData) => {
        ...
        payLoad.append('currentExperience', formData.currentExperience || '');
                                    ...
                                    jobData.map((item) => (
                                        <div key={item._id} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300">
                                            ...
                                            <div className="mt-4 flex justify-between">
                                                ...
                                                <button
                                                    onClick={() => handleApply(item._id, item)}  
