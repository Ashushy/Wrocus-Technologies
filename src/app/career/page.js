...
import ViewJobModal from '@/components/ViewJobModal'; // Import the new modal
import Sidebar from '@/components/Sidebar';

const Page = () => {
    const [jobData, setJobdata] = useState([]);
    const [selectedJob, setSelectedJob] = useState(null); // Track the selected job for viewing
    const [selectedJobId, setSelectedJobId] = useState(null);
    const [isApplyModalOpen, setIsApplyModalOpen] = useState(false);
    const [isViewModalOpen, setIsViewModalOpen] = useState(false); // Track whether the view modal is open
